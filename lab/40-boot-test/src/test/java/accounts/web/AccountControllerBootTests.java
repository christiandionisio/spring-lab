...
public class AccountControllerBootTests {
	...
	public void createAccount() throws Exception {
		...
		mockMvc.perform(post("/accounts")
			...
			.andExpect(redirectedUrl("http://localhost/accounts/21"));
	...
	public void accountSummaryTest() throws Exception {
		given(accountManager.getAllAccounts())
			.willReturn(List.of(new Account("001", "a"), new Account("002", "b")));
	...
	public void getBeneficiary() throws Exception {
		...
		account.addBeneficiary("AB");
		given(accountManager.getAccount(0L))
			.willReturn(account);

		mockMvc.perform(get("/accounts/{accountId}/beneficiaries/{beneficiaryName}", "0", "AB"))
			.andDo(print())
			.andExpect(status().isOk())
			.andExpect(content().contentType(MediaType.APPLICATION_JSON))
	...
	public void getBeneficiaryNotFound() throws Exception {
		...
		mockMvc.perform(get("/accounts/{accountId}/beneficiaries/{beneficiaryName}", "0", "ABC"))
			.andExpect(status().isNotFound());
	}
	...
	public void addBeneficiaryTest() throws Exception {
		...
		mockMvc.perform(post("/accounts/{accountId}/beneficiaries", "0")
				.content("AB"))
			.andExpect(status().isCreated())
			.andExpect(redirectedUrl("http://localhost/accounts/0/beneficiaries/AB"));
	}
	...
	public void removeBeneficiaryTest() throws Exception {
		...
		mockMvc.perform(delete("/accounts/{accountId}/beneficiaries/{beneficiaryName}", "0", "AB"))
			.andExpect(status().isNoContent());
	}

	@Test
	public void removeBeneficiaryTestNotFound() throws Exception {
		...
		mockMvc.perform(delete("/accounts/{accountId}/beneficiaries/{beneficiaryName}", "0", "ABC"))
			.andExpect(status().isNotFound());
	}
